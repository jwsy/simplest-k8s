...
2. The `values.yaml` file that contains a single line that sets `image: ghcr.io/jwsy/jade-shooter-22:v2.0.0`
...
https://itnext.io/simplest-minimal-k8s-app-tutorial-with-rancher-desktop-in-5-min-5481edb9a4a5
    ...
    2. `values.yaml` sets `image: ghcr.io/jwsy/jade-shooter-22:v2.0.0`
    3. `helm template .` uses the `Chart.yaml` and `values.yaml` in the current directory to substitute the value into the template and generate the K8s manifest that would be deployed 
        ...
        $ cat values.yaml
        image: ghcr.io/jwsy/jade-shooter-22:v2.0.0
...
5. Upgrade the app
    ...
    ```
    $ helm upgrade js . --set image=ghcr.io/jwsy/jade-shooter:v2.0.1
...
```bash
$ diff <(helm template js . --set image=ghcr.io/jwsy/jade-shooter-22:v2.0.0) <(helm template js . --set image=ghcr.io/jwsy/jade-shooter:v2.0.1)
```
...
<         image: ghcr.io/jwsy/jade-shooter-22:v2.0.0
---
>         image: ghcr.io/jwsy/jade-shooter:v2.0.1
```
